	...
	if (fse.existsSync(articlesDir)) {
		...
		for (let file of files) {
			...
			articles.push({
				...
				filename: `${filename.split(' ').join('-')}.html`,
				content: html,
...
module.exports = {
	...
	checkTheme: function() {
		...
		if (lacks.length > 0) {
			logger.error(`请检查主题目录是否符合规范: ${config.site.theme}`);
			logger.error('主题目录内缺少下列目录或文件:');
